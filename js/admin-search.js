...
( function ( $, wpapSearch ) {
	...
	function closePreview() {
		$subwindow.add( $subwindowOverlay ).fadeOut( 150, function () {
	...
	function createYahooUrl( keyword, sid, pid ) {
		var searchUrl =
			...
			encodeSearchUrl = encodeUrl( searchUrl );
		return (
			'//ck.jp.ap.valuecommerce.com/servlet/referral?sid=' +
			sid +
			'&pid=' +
			pid +
			'&vc_url=' +
	...
	$( document ).ready( function () {
		...
		$( document ).on(
			'click',
			'.display-preview .wpap-link-rakuten',
			function () {
				return false;
			}
		);
		...
		$( document ).on(
			'click',
			'.display-preview .wpap-link-yahoo',
			function () {
				return false;
			}
		);
		...
		$( document ).on(
			'click',
			'#wpap-admin-rakuten-ichiba-search',
			function () {
				var searchKeyword = $(
						'input[name="with_search_keyword"]'
					).val(),
					affiliateId = $(
						'input[name="rakuten_affiliate_id"]'
					).val(),
					affiliateUrl = createRakutenUrl(
						affiliateId,
						searchKeyword
					);
			...
			if ( $withSearchKeyword.is( '*' ) ) {
				var searchKeyword = $withSearchKeyword.val(),
					replaceSearchKeyword =
						replaceShortcodeParameter( searchKeyword );
		...
		$( document )
			.on( 'mouseenter', '[data-tooltip]', function () {
				...
				$( this ).append(
					'<span class="tooltip_hover"><span class="tooltip_hover_body">' +
						title +
						'</span></span>'
				);
			} )
			.on( 'mouseleave', '[data-tooltip]', function () {
				$( this ).find( 'span.tooltip_hover' ).fadeOut().remove();
			} );
		...
		$( document )
			.on(
				...
				function () {
					...
					$( this )
						.closest( '.display-preview' )
						.append(
							'<div class="wpap-balloon"><p>' +
								label +
								'</p></div>'
						);
				}
